...
class Feeder:
	...
	def __init__(self, coordinator, metadata_filename, hparams):
		...
		with tf.device('/cpu:0'):
			...
			print("debug windows checkpoint 1")
	...
	def make_test_batches(self):
		...
		log(strftime("---%a, %d %b %Y %H:%M:%S +0000", localtime()))
		log('feeder.py:make_test_batches():row151:Generated {} test batches of size {} in {:.3f} sec'.format(len(batches), n, time.time() - start))
		return batches, r
	...
	def _enqueue_next_train_group(self):
		while not self._coord.should_stop():
			...
			print('feeder.py:_enqueue_next_train_group():row168:Generated {} train batches of size {} in {:.3f} sec'.format(len(batches), n, time.time() - start))
			for batch in batches:
				...
				#print('feeder.py:_enqueue_next_train_group():row174')
	...
	def _enqueue_next_test_group(self):
		...
		while not self._coord.should_stop():
			for batch in test_batches:
				...
				print('feeder.py:_enqueue_next_test_group():row182')

	def _get_next_example(self):
		"""Gets a single example (input, mel_target, token_target, linear_target, mel_length) from_ disk
		"""
		#print('{}:feeder.py:_get_next_example():row190.'.format(strftime("---%a, %d %b %Y %H:%M:%S +0000", localtime())))
	...
	def _prepare_batch(self, batches, outputs_per_step):
		#log('feeder.py:_prepare_batch():row205')
		...
		#log('feeder.py:_prepare_batch():row236')
