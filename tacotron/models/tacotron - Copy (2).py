...
class Tacotron():
	...
	def initialize(self, inputs, input_lengths, mel_targets=None, stop_token_targets=None, linear_targets=None, targets_lengths=None, gta=False,
		...
		#	with tf.device(tf.train.replica_device_setter(ps_tasks=1,ps_device="/cpu:0",worker_device=gpus[i])):
			with tf.device(tf.train.replica_device_setter(ps_tasks=1,ps_device="/cpu:0",worker_device="/cpu:0")): # debug using gpu:0 will cause OOM, use >1640MB
				with tf.variable_scope('inference') as scope:
					...
					print("debug enc_conv_output_shape:",enc_conv_output_shape)

			with tf.device(tf.train.replica_device_setter(ps_tasks=1,ps_device="/cpu:0",worker_device="/cpu:0")): # debug try to use as more gpu as possible
				with tf.variable_scope('inference') as scope:
			...
			with tf.device(tf.train.replica_device_setter(ps_tasks=1,ps_device="/cpu:0",worker_device="/gpu:0")): # debug try to use as more gpu as possible
				with tf.variable_scope('inference') as scope:
	...
	def add_loss(self):
		...
		for i in range(hp.tacotron_num_gpus):
			#with tf.device(tf.train.replica_device_setter(ps_tasks=1,ps_device="/cpu:0",worker_device=gpus[i])):
			with tf.device(tf.train.replica_device_setter(ps_tasks=1,ps_device="/cpu:0",worker_device="/cpu:0")): # debug gpu:0 will use about 254MB memory
	...
	def add_optimizer(self, global_step):
		...
		with tf.device("/cpu:0"): # debug gpu0 is nvidia 960m, use about 128MB memory
		...
		for i in range(hp.tacotron_num_gpus):
			#  Device placement
			#with tf.device(tf.train.replica_device_setter(ps_tasks=1,ps_device="/cpu:0",worker_device=gpus[i])) :
			with tf.device('/cpu:0') : # debug, using gpu:0 will cause OOM, use >1550MB memory
		...
		# 3. Average Gradient
		#with tf.device(grad_device) :
		with tf.device("/cpu:0") : # debug gpu0 is nvidia 960m, use about 382MB memory
			avg_grads = []
			vars = []
