	...
	with tf.Session(config=config) as sess:
		...
		try:
			...
			if args.restore:
				...
				try:
					log("train.py:train():row198:before calling get_checkpoint_state()")
					checkpoint_state = tf.train.get_checkpoint_state(save_dir)

					if (checkpoint_state and checkpoint_state.model_checkpoint_path):
						log('Loading checkpoint {}'.format(checkpoint_state.model_checkpoint_path), slack=True)
						print("---train.py:train():row202:after loading checkpoint")
			...
			print("train.py:train():row219:after calling feeder.start_threads()")
			#Training loop
			while not coord.should_stop() and step < args.tacotron_train_steps:
				start_time = time.time()
				#print("\ntrain.py:train():row223:before calling sess.run...().step:{}".format(step))
				step, loss, opt = sess.run([global_step, model.loss, model.optimize])
				#print("train.py:train():row225:after calling sess.run...().step:{}".format(step))
				...
				if step % args.checkpoint_interval == 0 or step == args.tacotron_train_steps or step == 300:
					...
					log('{}:train.py:train():row313.'.format(strftime("---%a, %d %b %Y %H:%M:%S +0000", localtime())))
					message = 'Step {:7d} [{:.3f} sec/step, loss={:.5f}, avg_loss={:.5f}]'.format(
						step, time_window.average, loss, loss_window.average)
					log(message, end='\r', slack=(step % args.checkpoint_interval == 0))
